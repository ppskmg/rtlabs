...
//		w.WriteHeader(200)
//	})
//	chained := Middlewares().ThenFunc(fn)
...
//	t3 := tagMiddleware("t3\n")
//
//	chained := Middlewares(t1, t2, t3).Then(testApp)
//
...
//	chain := Middlewares(tagMiddleware("t1\n"), tagMiddleware("t2\n"))
...
//	chain := Middlewares(tagMiddleware(""))
...
//	chain1 := Middlewares(tagMiddleware("t1\n"), tagMiddleware("t2\n"))
//	chain2 := Middlewares(tagMiddleware("t3\n"), tagMiddleware("t4\n"))
...
//	chain := Middlewares(tagMiddleware(""))
//	newChain := chain.Extend(Middlewares(tagMiddleware("")))
//
//	assert.NotEqual(t, &chain.constructors[0], &newChain.constructors[0])
//}
